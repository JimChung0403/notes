...
### 如果 `node ./node_modules/@playwright/mcp/cli.js` 不能用

這通常表示以下其中一種情況：

1. 你的專案裡還沒安裝 `@playwright/mcp`
2. `@playwright/mcp` 不是裝在目前這個專案
3. 套件有裝，但實際位置不是 `./node_modules/@playwright/mcp/cli.js`
...
如果有裝，通常會看到套件版本；如果沒有裝，通常會看到空、missing 或非 0 exit code。
...
```bash
npm ls -g @playwright/mcp --depth=0
```
...
```bash
npm root -g
find "$(npm root -g)" -path '*/@playwright/mcp/cli.js' 2>/dev/null
```

### 判斷規則

- 專案內找得到 `./node_modules/@playwright/mcp/cli.js` → 優先用專案內版本
- 專案內找不到，但全域找得到 → 可用全域版本
- 專案內、全域都找不到 → 代表你目前沒有 `@playwright/mcp`

#### 情境 1：你們公司可以 `npm install`

如果你們公司內網可以直接安裝 npm 套件，建議在專案根目錄安裝：
...
#### 情境 2：你已經有 `playwright`，但沒有 `@playwright/mcp`
...
只代表你安裝了 Playwright 套件，不代表你已經安裝 Playwright MCP server。MCP server 的官方套件名稱是 `@playwright/mcp`，官方文件的標準配置也是透過 `npx @playwright/mcp@latest` 啟動它。換句話說，**`playwright` 和 `@playwright/mcp` 是兩件事**。

所以如果你要讓 `Claude Code` 或 `OpenCode` 連 MCP，請補安裝：

```bash
npm install @playwright/mcp
```

#### 情境 3：專案本身也要寫 Playwright 腳本

如果你除了 MCP 之外，也想在這個 repo 裡寫一般 Playwright 自動化程式，可以兩個都裝：

```bash
npm install playwright @playwright/mcp
```

#### 情境 4：你不想用專案內安裝，想用全域安裝
...
然後把 `.mcp.json` 或 `opencode.json` 裡的路徑改成實際位置。

### 最推薦的做法

對你目前的情境，最推薦是直接在專案內安裝：
...
- `playwright` 給你一般 Playwright 能力
- `@playwright/mcp` 給 `Claude Code` / `OpenCode` 使用的 MCP server
- 版本跟著專案走，比全域安裝容易管理

裝完後先確認：

```bash
npm ls playwright @playwright/mcp --depth=0
find . -path '*/node_modules/@playwright/mcp/cli.js' 2>/dev/null
```

### 安裝後要不要再跑 `playwright install`

在你目前這個情境，**不是優先必要步驟**。

因為你已經有一個可用的 Chromium executable，並且 SOP 是用：

- `launchOptions.executablePath`

直接指定現成 browser binary。

所以你的優先順序應該是：

1. 先安裝 `@playwright/mcp`
2. 先確認 `cli.js` 找得到
3. 先確認 `executablePath` 指到你已有的 browser
4. 只有在你改成依賴 Playwright 自己管理的 browser bundle 時，才再考慮 `playwright install`

---

## 8. 第一次安裝後怎麼驗證

安裝完 `playwright` 和 `@playwright/mcp` 後，不要直接跳進 AI 操作流程。先做一次最小驗證。
...
### 驗證 2：browser executable 存不存在

```bash
test -x /home/mcchungp/Documents/project/tool/playwright2/ms-playwright/chromium-1194/chrome-linux/chrome && echo OK
```

如果沒有印出 `OK`，先不要往下做，先修正 `executablePath`。

### 驗證 3：config 檔有沒有指到正確路徑

確認 `tools/playwright-mcp.config.json` 至少有：
...
### 驗證 4：直接手動啟動 MCP server

先不要透過 Claude Code / OpenCode，先直接起一次：

```bash
node ./node_modules/@playwright/mcp/cli.js --config ./tools/playwright-mcp.config.json
```

如果這一步直接報錯，先修這一步，不要往後排查 AI CLI。

### 驗證 5：Claude Code 能不能看到 MCP
...
如果設定正確，應該看得到 `playwright-local`。

### 驗證 6：OpenCode 能不能看到 MCP
...
如果它能列出 `playwright-local`，表示 MCP 設定至少有被吃到。

### 驗證 7：最小功能測試

第一次只測一件事：**AI 能不能成功開 browser**。

例如在 AI CLI 中下這種指令：

```text
請使用 playwright-local 開啟 about:blank，然後回報目前頁面標題。
```

如果能成功，下一步再測：

```text
請使用 playwright-local 開啟 https://example.com，等待頁面穩定，然後告訴我頁面標題。
```

### 驗證 8：如果公司環境要靠 proxy 才能對外

如果 `about:blank` 可以，但公開網站不行，優先懷疑：

- proxy 沒帶進 Playwright MCP
- `ProxyPacUrl` / `ProxyServer` 沒有複製進 config
- `--proxy-*` flags 沒有套上

這時回去看第 11 節的 proxy 處理。

### 推薦驗證順序

1. 套件存在
2. `cli.js` 存在
3. `executablePath` 可執行
4. MCP server 可手動啟動
5. AI CLI 看得到 MCP
6. 先測 `about:blank`
7. 再測公開網站
...
## 9. Claude Code + Playwright MCP SOP
...
## 10. OpenCode + Playwright MCP SOP
...
## 12. 如果公司 Chrome 是靠 Proxy 才能對外

這一節很重要，因為你現在遇到的狀況就是：

- 公司平常用的 Chrome 可以對外
- Playwright 啟動出來的 Chromium 只能打 `127.0.0.1`

這通常代表 **Playwright 啟動的 browser instance 沒有吃到公司那套 proxy 設定**。

### 先查公司 Chrome 的 proxy 來源

先在「那個可以正常對外的公司 Chrome」裡檢查這兩個頁面：
...
如果有，表示你們很可能是 **企業 policy** 在控管 proxy。

#### 看 `chrome://version`

重點找 command line 裡有沒有：

- `--proxy-server=...`
- `--proxy-pac-url=...`
- `--proxy-bypass-list=...`

如果有，表示公司那個 Chrome 是靠啟動參數或 wrapper script 套 proxy。

### 原則

你的目標不是「猜系統 proxy」，而是：

1. 先找出公司 Chrome 能上外網的依據
2. 再把同一組設定搬進 Playwright MCP 的 `launchOptions`

### 方式 A：明確 proxy server

如果你查到的是固定 proxy server，例如：
  ...
  "browser": {
    ...
    "launchOptions": {
      ...
      "proxy": {
        ...
        "bypass": "127.0.0.1,localhost,.corp.local"
      }
    }
  }
}
...
### 方式 C：公司 Chrome 本來就是靠 flags 啟動

如果你在 `chrome://version` 看到了：

```text
--proxy-server=http://proxy.company.com:8080
--proxy-bypass-list=127.0.0.1;localhost;.corp.local
```

那就直接複製到 Playwright MCP：
...
- **先抄公司 Chrome 的既有 proxy 行為**，不要自己猜
- 如果 `chrome://policy` 已有值，優先照 policy 值做
- 如果 `chrome://version` 有 `--proxy-*`，就直接照抄
- 要保留登入狀態時，持續使用自己的 `userDataDir`
- 不要直接接管你平常人工作業的主 profile

### 你的排查順序

1. 用公司可對外的 Chrome 開 `chrome://policy`
2. 記下 `ProxyMode / ProxyServer / ProxyPacUrl / ProxyBypassList`
3. 再看 `chrome://version`
4. 把查到的 proxy 設定帶進 `tools/playwright-mcp.config.json`
5. 用 Playwright MCP 測一個外部網站，例如公司允許的公開站點
